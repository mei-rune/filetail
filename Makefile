...
build: depend clean test
	...
	gox -ldflags=$(GOLDFLAGS) -os="$(X64_PLATFORMS)" -arch="amd64" -output "build/{{.OS}}/amd64/filetail/filetail"
	gox -ldflags=$(GOLDFLAGS) -os="$(X86_PLATFORMS)" -arch="386" -output "build/{{.OS}}/i386/filetail/filetail"
	gox -ldflags=$(GOLDFLAGS) -os="linux" -arch="arm" -output "build/linux/armhf/filetail/filetail"
	gox -ldflags=$(GOLDFLAGS) -cgo -os="$(CGO_PLATFORMS)" -arch="amd64" -output "build/{{.OS}}/amd64/filetail/filetail"
...
$(BUILD_PAIRS): build
	...
	$(eval ARCH := $(notdir $@))
	mkdir pkg || echo
	cp $(BUILD_DOCS) build/$@/filetail

	if [ "$(PLATFORM)" = "linux" ]; then\
		mkdir -p pkg/tmp/etc/init.d;\
		mkdir -p pkg/tmp/usr/local/bin;\
		cp -f example_config.yml pkg/tmp/etc/log_files.yml;\
		cp -f packaging/linux/filetail.initd pkg/tmp/etc/init.d/filetail;\
		cp -f build/$@/filetail/filetail pkg/tmp/usr/local/bin;\
		(cd pkg && \
		fpm \
		  -s dir \
		  -C tmp \
		  ...
		  --config-files etc/init.d/filetail usr/local/bin/filetail etc/log_files.yml etc/init.d/filetail && \
		fpm \
		  -s dir \
		  -C tmp \
		  -t rpm \
		  ...
		  --config-files etc/log_files.yml \
		  --config-files etc/init.d/filetail \
		  --rpm-os linux usr/local/bin/filetail etc/log_files.yml etc/init.d/filetail );\
		rm -R -f pkg/tmp;\
	fi

	cd build/$@ && echo `pwd` && tar -cvzf ../../../pkg/filetail_$(PLATFORM)_$(ARCH).tar.gz filetail
